...
static int	verif_first_pipe_error(char *str, size_t *i, int *pipe_last,
	int first_char)
{
	if (first_char ||
...
static int	verif_wrong_sep(int *pipe_last, int *first_char)
{
	if (*pipe_last)
		return (0);
	*first_char = 1;
	...
	while (str[i])
	{
		...
		if (!is_escaped_char(str, i) && is_quote_open(str[i]))
			...
			continue ;
				...
				&& !verif_wrong_sep(pipe_last, &first_char))
		...
		else if (!verif_first_pipe_error(str, &i, pipe_last, first_char))
			return (print_pipe_error());
	}
	return (0);
}
